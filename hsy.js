 ...
 * 合生元官方商城  小程序
 *
 * cron 0 0,7 * * *  yml2213_javascript_master/hsy.js
 *
 * 8-14		签到兑换实物
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export hsy_data=' extra-data '   ,多账号用 换行 或 @ 分割
 ...
 */
 ...
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.hsy_data : $.getdata('hsy_data')) || '';
 let msg, ck;
 let ck_status = true;
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "hsy_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 let url = {
			 url: `${hostname}/wscump/checkin/checkinV2.json?checkinId=26655&app_id=wx0546daaba85b7ebc&kdt_id=92200556`,
			 headers: {
				 "Host": host,
