 ...
 */
 ...
 let msg, ck;
 let ck_status = true;
 let host = 'httpapi-prd.shinho.net.cn';
 let hostname = 'https://' + host;
 
 //---------------------------------------------------------------------------------------------------------
 let Change = '每天签到，兑换调料'
 let thank = `\n感谢 群友 的投稿\n`
 //---------------------------------------------------------------------------------------------------------
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "xinhe_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 ...
		 if (result?.code == 1000) {
			...
			DoubleLog(`签到:${result?.message}  ,请勿重复签到`);
			await wait(3);
		} else {
 ...
 async function jifen() {
	 try {
		 ...
		 if (result?.code == 1000) {
			 DoubleLog(`积分查询:${result?.message} ,当前积分${result?.data.userInfo.integral}🎉`);
			 await wait(3);
		 }  else {
