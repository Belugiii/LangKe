...
async function userinfo(a, b) {
    try {
        ...
        1 == c.state && (this.cash = c.data.money_zc, console.log(jsname + ` 当前金币 ：${this.cash} `), await timeMoney(a, b)), this.cash >= 998 && (console.log(jsname + ` 开始提现 ：${number(1e-4*this.cash)} 元`), await addMoney(a, b))
    } catch (d) {
        console.log(d)
    } finally {
...
async function addMoney(a, b) {
    try {
        let e;
        await httpRequest("get", populateUrlObject(`https://json.dd-gz.com/zhuandianshidian/public/api.php/user/addMoney?access=i20180418&appid=2021002117637233&edition=190001&price=${this.cash}&token=${a}&userid=${b}`, ""));
        let c = httpResult;
        1 == c.state && console.log(jsname+` 提现状态: 兑换成功`)
          !1== c.state && console.log(jsname+ ` 提现状态: ${c.msg}`)
    } catch (d) {
        console.log(d)
    } finally {
        return new Promise(a => {
...
'alipayMiniMark' : `Haeyotx/nVfal1tXljAKxON5OI8aIQQ3WheM1km1GI/k6Jqr6+0WIr19c4bUo7gv4Y6W7TScHMCgnkfasNSNrXfL8Hgyc+C233+jklECVVk=`,
'User-Agent' : `Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/18H17 Ariver/1.1.0 AliApp(AP/10.2.56.6000) Nebula WK RVKType(0) AlipayDefined(nt:WIFI,ws:414|832|2.0) AlipayClient/10.2.56.6000 Language/zh-Hans Region/CN NebulaX/1.0.0`,
'Cookie' : ``,
'Referer' : `https://2021002117637233.hybrid.alipay-eco.com/2021002117637233/0.2.2208111755.38/index.html#pages/userduihuan/user?__appxPageId=3`,
'Host' : `json.dd-gz.com`,
