...
// 判断环境变量里面是否有京东ck
if (process.env.JSB_COOKIE) {
  if (process.env.JSB_COOKIE.indexOf('&') > -1) {
    CookieJDs = process.env.JSB_COOKIE.split('&');
  } else if (process.env.JSB_COOKIE.indexOf('\n') > -1) {
    CookieJDs = process.env.JSB_COOKIE.split('\n');
  } else {
    CookieJDs = [process.env.JSB_COOKIE];
  }
}
