...
let Notify = 0;
let ps = `
    提示: 抓取[${$.name}]小程序(https://api-c-prod.etcp.org.cn)请求头中的userId和token,变量名:${ckName},多个账号使用@或换行分隔!!! 示例: userId&token
`
let envSplitor = ["@", "\n", "\r\n"]; //多账号分隔符
let userIdx = 0;
let userList = [];
let ua = 'Mozilla/5.0 (Linux; Android 11; MI 9 Build/RKQ1.200826.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/126.0.6478.187 Mobile Safari/537.36 XWEB/1260075 MMWEBSDK/20231202 MMWEBID/209 MicroMessenger/8.0.47.2560(0x28002F51) WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64 MiniProgramEnv/android'
class Task {
    constructor(str) {
        this.index = ++userIdx;
        this.userId = str.split('&')[0];
    ...
    async taskRequest(method, url, header = "", body = "") {
        ...
        let headers = { 
            ...
            'charset': 'utf-8', 
            'openid': 'oJ0855CrfVxQkE6-NP7UWz8wG6HM', 
            'version': '', 
            ...
            'token': this.token, 
            'Referer': 'https://servicewechat.com/wxd07028ca81227e3b/2/page-frame.html'
          }
    ...
    async init() {
        try {
            let result = await this.taskRequest("get", `https://api-c-prod.etcp.org.cn/carbonAssets/account/v1/select?userId=${this.userId}`)
            if(result.code != 0) return;
            this.accountNo = result.data.accountNo;
            this.balance = result.data.balance;
            this.ckStatus = true;
            $.log(`用户名:${this.accountNo}\n余额:${this.balance}`)
        } catch (e) {
            console.log(e);
            this.ckStatus = false;
        }
    ...
    async sign() {
        try {
            console.log("🔰   ==>   开始签到")
            let result = await this.taskRequest("post", `https://api-c-prod.etcp.org.cn/carbonAssets/sign/in/v1/today`, {}, `{"userId":${this.userId}}`)
            if(result.code != 0){
                $.log(result.message);
            }else{
                $.log("✔️   <==   签到成功");
