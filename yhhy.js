 ...
 * 洋河会员中心  小程序
 *
 * cron 0 0,7 * * *  yml2213_javascript_master/yanghe.js
 *
 * 7-7		签到
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export yanghe_data=' memberid & authorization '   ,多账号用 换行 或 @ 分割
 * 
 */
...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.yanghe_data : $.getdata('yanghe_data')) || '';
let msg, ck;
let ck_status = true;
let host = 'user.chinayanghe.com';
...
!(async () => {
	let ckArr = await checkEnv(ckStr, "yanghe_data");
	await tips(ckArr);
...
async function signin() {
	try {
		let url = {
			url: `${hostname}/gateway/api-base/checkIn/memberCheckIn?memberId=`+ck[0],
			headers: {
			'Host': host,
			'authorization': ck[1],
			},
