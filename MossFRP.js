const $ = new Env("MossFRP");
const notify = $.isNode() ? require('./sendNotify') : '';
let ckName = "moss";
let Notify = 0;
let ps = `
	提示: 填写[${$.name}]网页(https://www.mossfrp.top)的用户名和密码,变量名:${ckName},多个账号使用@或换行分隔!!! 示例: username&password
`
let envSplitor = ["\n", "\r\n"]; //多账号分隔符
let userIdx = 0;
let userList = [];
let ua = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36 Edg/114.0.1823.43'
...
class Task {
    ...
    async init() {
        try {
            let result = await this.taskRequest("post", `https://https.ghs.wiki:7002/API?void=post`,{},{"type": "login","loginType": "email","account": this.account,"password": CryptoJS.SHA256(this.password).toString(),"encryption": "true"})
            if(result.status != 200) throw new Error('获取用户token失败,请检查用户名或密码!!😭');
    ...
    async info() {
        try {
            let result = await this.taskRequest("post", `https://https.ghs.wiki:7002/API?void=post`, {}, {"type": "userInfo","token": this.token})
            if(result.status != 200){
                
            }else{
                this.silver = result.userInfo.silver;
            }
        } catch (e) {
            Notify++;
            console.log(e);
    ...
    async sign() {
        try {
            let result = await this.taskRequest("post", `https://https.ghs.wiki:7002/API?void=post`, {}, {"type": "signIn","token": this.token})
            if(result.status != 200){
