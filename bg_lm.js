...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.lm_data : $.getdata("lm_data")) || "";
let msg, ck;
// let ck_status = true;
...
!(async () => {
	if (typeof $request !== "undefined") {
		...
		let ckArr = await Variable_Check(ckStr, "lm_data");
		await tips(ckArr);
...
async function GetRewrite() {
	if ($request.url.indexOf("api/mp2c/user/profile") > -1) {
		const ck = $request.headers['Cookie']
		if (ckStr) {
			if (ckStr.indexOf(ck) == -1) { // 找不到返回 -1
				ckStr = ckStr + "@" + ck;
				$.setdata(ckStr, "lm_data");
				ckList = ckStr.split("@");
