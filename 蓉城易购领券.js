...
let Notify = 0;
let ps = `
    提示: 抓取[${$.name}]小程序(https://client.028rcyg.com)请求头中的Authorization,变量名:${ckName},多个账号使用@或换行分隔!!! 
`
let envSplitor = ["@", "\n", "\r\n"]; //多账号分隔符
let userIdx = 0;
let userList = [];
const qs = require('qs');
let ua = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090a13) UnifiedPCWindowsWechat(0xf254061a) XWEB/14307'
class Task {
    constructor(str) {
        this.index = ++userIdx;
        this.token = str;
    ...
    async taskRequest(method, url, header = "", body = "") {
		...
		'Referer': 'https://servicewechat.com/wx7c922a640269e2ea/149/page-frame.html', 
    ...
    async init() {
        try {
            let result = await this.taskRequest("get", `https://client.028rcyg.com/api/customer/getNowClient`);
    ...
    async receiveCoupons() {
        try {
            let result = await this.taskRequest("get", `https://client.028rcyg.com/api/marketingCoupon/selectCouponList?couponId`)
            if(result.code != 200){
                $.log(`获取优惠券列表失败xxx`);
				return;
            }
			...
			for(let row of rows){
			  if(!row.isReceive){
				  result = await this.taskRequest("get", `https://client.028rcyg.com/api/marketingCoupon/getCoupon?couponId=${row.couponId}`);
				  if(result.code != 200){
					  $.log(`优惠券:${row.instructions} ${row.name} 领取失败!!!`);
				  }else{
					  $.log(`优惠券:${row.instructions} ${row.name} 领取成功!!!`);
				  }
			  }
			  await $.wait(2 * 1000);
			}
