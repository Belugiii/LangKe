...
let Notify = 0;
let ps = `
    提示: 抓取[${$.name}]小程序(https://member-api.icoke.cn)请求头中的authorization,变量名:${ckName},多个账号使用@或换行分隔!!! 
`
let envSplitor = ["@", "\n", "\r\n"]; //多账号分隔符
let userIdx = 0;
let userList = [];
...
class Task {
    ...
    async main() {
        ...
        $.log(`用户名: ${this.name} 手机号: ${this.mobile}`);
        await this.sign();
    }
    async taskRequest(method, url, header = "", body = "") {
        ...
        let headers = { 
			...
			'referer': 'https://servicewechat.com/wxa5811e0426a94686/472/page-frame.html', 
    ...
    async init() {
        try {
            let result = await this.taskRequest("get", `https://member-api.icoke.cn/api/icoke-customer/icoke/mini/customer/main/base/info`);
    ...
    async sign() {
        try {
            let result = await this.taskRequest("get", `https://member-api.icoke.cn/api/icoke-sign/icoke/mini/sign/main/getSignOutline`)
            if(!result.success){
