...
def get_wskey():  # 方法 获取 wskey值 [系统变量传递]
    if "JSB_WSCK" in os.environ:  # 判断 JSB_WSCK是否存在于环境变量
        wskey_list = os.environ['JSB_WSCK'].split('&')  # 读取系统变量 以 & 分割变量
        if len(wskey_list) > 0:  # 判断 WSKEY 数量 大于 0 个
            return wskey_list  # 返回 WSKEY [LIST]
        else:  # 判断分支
            logger.info("JSB_WSCK变量未启用")  # 标准日志输出
            sys.exit(1)  # 脚本退出
    else:  # 判断分支
        logger.info("未添加JSB_WSCK变量")  # 标准日志输出
        sys.exit(0)  # 脚本退出


# 返回值 list[JSB_COOKIE]
def get_ck():  # 方法 获取 JSB_COOKIE值 [系统变量传递] <! 此方法未使用 !>
    if "JSB_COOKIE" in os.environ:  # 判断 JSB_COOKIE是否存在于环境变量
        ck_list = os.environ['JSB_COOKIE'].split('&')  # 读取系统变量 以 & 分割变量
        if len(ck_list) > 0:  # 判断 WSKEY 数量 大于 0 个
            return ck_list  # 返回 JSB_COOKIE [LIST]
        else:  # 判断分支
            logger.info("JSB_COOKIE变量未启用")  # 标准日志输出
            sys.exit(1)  # 脚本退出
    else:  # 判断分支
        logger.info("未添加JSB_COOKIE变量")  # 标准日志输出
        sys.exit(0)  # 脚本退出
...
def serch_ck(pin):  # 方法 搜索 Pin
    for i in range(len(envlist)):  # For循环 变量[envlist]的数量
        if "name" not in envlist[i] or envlist[i]["name"] != "JSB_COOKIE":  # 判断 envlist内容
            continue  # 继续循环
...
def ql_insert(i_ck):  # 方法 插入新变量
    data = [{"value": i_ck, "name": "JSB_COOKIE"}]  # POST数据载荷组合
    data = json.dumps(data)  # Json格式化数据
    url = ql_url + 'api/envs'
    s.post(url=url, data=data)  # HTTP[POST]请求 使用session
    logger.info("\n账号添加完成\n--------------------\n")  # 标准日志输出
    ...
    for ws in wslist:  # wslist变量 for循环  [wslist -> ws]
        ...
        if "pin" in wspin:  # 判断 pin 是否存在于 [wspin]
            ...
            if return_serch[0]:  # bool: True 搜索到账号
                jck = str(return_serch[1])  # 拿到 JSB_COOKIE
                if not check_ck(jck):  # bool: False 判定 JSB_COOKIE 有效性
                    tryCount = 1  # 重试次数 1次
                    ...
                    for count in range(tryCount):  # for循环 [tryCount]
                        count += 1  # 自增
                        return_ws = getToken(ws)  # 使用 WSKEY 请求获取 JSB_COOKIE bool jd_ck
                    ...
                    if return_ws[0]:  # 判断 [return_ws]返回值 Bool类型
                        ...
                        ql_update(eid, nt_key)  # 函数 ql_update 参数 eid JSB_COOKIE
                    else:  # 判断分支
