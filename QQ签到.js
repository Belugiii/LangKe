...
青龙：
1.抓包 https://ti.qq.com , 找到 请求头中的cookie，填写到qqck,多账号用 换行符 分割
...
class UserInfo {
    ...
    async signin() {
        try {
            let ck = this.headers.Cookie;
            let arr = ck.split("; ");
            let map = new Map();
            for(let i in arr){
	            let ckarr = arr[i].split("=");
	            map.set(ckarr[0],ckarr[1]);
            }
            let uin = map.get("uin");
            uin = uin.replace(/o0*/,"")
            this.body = JSON.stringify({
			  ...
			  "mpExtend": {
			    "tianshuAdsReq": "{\"app\":\"QQ\",\"os\":\"Android\",\"version\":\"9.0.17\",\"imei\":\"\"}"
			  }
			});
			const options = {
                //签到任务调用签到接口
