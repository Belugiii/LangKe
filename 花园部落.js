...
let Notify = 0;
let ps = `
    提示: 抓取[${$.name}]小程序(https://m.mallcoo.cn)请求体中的MallID和Token,变量名:${ckName},多个账号使用@或换行分隔!!! 示例: MallID&Token 注:Token中的逗号和数字都不能少
`
let envSplitor = ["@", "\n", "\r\n"]; //多账号分隔符
let userIdx = 0;
let userList = [];
let separator = '&'; // 用于分割多个参数
let ua = 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.44(0x18002c2f) NetType/WIFI Language/zh_CN'
class Task {
    constructor(str) {
        this.index = ++userIdx;
        this.mallId = str.split(separator)[0];
        this.token = str.split(separator)[1];
        this.ckStatus = false;
        this.userName,this.totalBonus;
    }
    ...
    async taskRequest(method, url, header = "", body = "") {
        ...
        let headers = {
            ...
            'User-Agent': ua,
            'Referer': 'https://servicewechat.com/wxcf30f7160c8cb3c6/41/page-frame.html'
        }
        ...
        body = {"MallID": this.mallId,"Header": {"Token": this.token}};
    ...
    async init() {
        try {
            let result = await this.taskRequest("post", `https://m.mallcoo.cn/api/user/user/GetMallCard`);
            if (result.m != 1) throw new Error('获取用户信息失败,请检查token是否失效!!😭');
            this.userName = result.d.UserName;
            this.totalBonus = result.d.TotalBonus;
            this.ckStatus = true;
        } catch (e) {
    ...
    async sign() {
        try {
            let result = await this.taskRequest("post", `https://m.mallcoo.cn/api/user/User/CheckinV2`)
            if (result.m != 1) {
                $.log(`签到: 签到失败❌ 原因: ${result.e}`);
            } else {
                $.log(`签到: 签到成功✔️  获得${result.d.Content}积分`);
                if(result.d.Content) this.totalBonus += parseInt(result.d.Content);
            }
        } catch (e) {
            Notify++;
            console.log(e);
