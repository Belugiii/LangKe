 ...
 * 杰士邦安心福利社  小程序
 *
 * cron 0 0,7 * * *  yml2213_javascript_master/dz.js
 *
 * 7-6 签到，仅支持青龙
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export jsb_data=' access-token的值 '   ,多账号用 换行 或 @ 分割
 * 
 */
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "jsb_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 let url = {
			 url: `${hostname}/index.php?s\u003d%2Fapi%2Fsign%2Fapply`,
			 headers: {
				 'Host': host,
				 'access-token': ck[0],
			 },
