 ...
 */
 ...
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.xybc_data : $.getdata('xybc_data')) || '';
 let msg, ck;
 let ck_status = true;
 let host = 'crm.inoherb.com';
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "xybc_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 let url = {
			 url: `${hostname}/api/social/write/sign/doSign.do`,
			 headers: {
			//	"Host": host,
			//	"cookie": `ut=${ck[0]}`,
		 ...
		 //console.log(result);
		 if (result?.data.message == '签到成功') {
			 DoubleLog(`签到成功 🎉，签到获得:${result.data.amount} 积分`);
			 await wait(3);
		 } else if (result?.data.amount == null) {
			 DoubleLog(`签到:${result.data.message},请勿重复签到`);
		 } else {
