...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.XYJ_token : $.getdata("XYJ_token")) || "";
let msg, ck;
...
!(async () => {
	if (typeof $request !== "undefined") {
		...
		let ckArr = await Variable_Check(ckStr, "XYJ_token");
		await tips(ckArr);
...
async function GetRewrite() {
	if ($request.url.indexOf("api/xiyuejiawxapp/getuserdata") > -1) {
	
const ck = $request.url.match(/openid=(.*)/)[1]
		if (ckStr) {
			if (ckStr.indexOf(ck) == -1) { // 找不到返回 -1
				ckStr = ckStr + "@" + ck;
				$.setdata(ckStr, "XYJ_token");
				ckList = ckStr.split("@");
				$.msg($.name + ` 获取第${ckList.length}个 ck 成功: ${ck} ,不用请自行关闭重写!`);
			}
		} else {
			$.setdata(ck, "XYJ_token");
...
function xyjsp(timeout = 3 * 1000) {
	return new Promise((resolve) => {
		let url = {
			url: `https://midea.design0759.com/index.php/api/xiyuejiawxapp/addvideo?openid=${ck[0]}`,
headers: {
'Referer' : `https://servicewechat.com/wx804dccc51baf570c/1/page-frame.html`,
'Connection' : `keep-alive`,
'Host' : `midea.design0759.com`,
'Content-Type' : `application/json`,
'Accept-Encoding' : `gzip,compress,br,deflate`,
'User-Agent' : `Mozilla/5.0 (iPhone; CPU iPhone OS 14_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.22(0x1800162a) NetType/WIFI Language/zh_CN`
}
	    }
...
function xyjwz(timeout = 3 * 1000) {
	return new Promise((resolve) => {
		let url = {
			url: `https://midea.design0759.com/index.php/api/xiyuejiawxapp/viewtuwen?openid=${ck[0]}`,
headers: {
'Referer' : `https://servicewechat.com/wx804dccc51baf570c/1/page-frame.html`,
'Connection' : `keep-alive`,
'Host' : `midea.design0759.com`,
...
function xyjcj(timeout = 3 * 1000) {
	return new Promise((resolve) => {
		let url = {
			url: `https://midea.design0759.com/index.php/api/xiyuejiawxapp/cj?openid=${ck[0]}`,
headers: {
'Referer' : `https://servicewechat.com/wx804dccc51baf570c/1/page-frame.html`,
'Connection' : `keep-alive`,
'Host' : `midea.design0759.com`,
