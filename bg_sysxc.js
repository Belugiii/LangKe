 ...
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export sxc_data=' auth & memberId '   ,多账号用 换行 或 @ 分割
 * 抓scrm-prod.shuyi.org.cn下auth和memberid
 * 积分兑换10-2的券，有需要再上
 */
 ...
 let msg, ck;
 let ck_status = true;
 let host = 'scrm-prod.shuyi.org.cn';
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "sxc_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 let url = {
			 url: `${hostname}/saas-gateway/api/agg-trade/v1/signIn`,
			 headers: {
				 "Host": host,
		 ...
		 if (result?.resultCode == 0) {
			 DoubleLog(`签到:${result?.resultMsg} 🎉,签到获得:${result?.data.rewardValue}积分 `);
			 await wait(3);
  ...
  */
   async function jifen() {
	try {
		let url = {
			url: `${hostname}/saas-gateway/api/agg-trade/v1/member/points/list?pageNum=1&pageSize=10&type=0&memberId=${ck[1]}&isQueryWillExpire=0`,
			headers: {
				 "Host": host,
				 "hostname": "scrm-prod.shuyi.org.cn",
				 "auth": ck[0],
		...
		if (result?.resultCode == 0) {
			DoubleLog(`查询积分:${result?.resultMsg} 🎉,当前积分:${result?.extFields.availablePoints} `);
			await wait(3);
		} else {
